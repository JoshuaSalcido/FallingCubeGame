...
public class MoveBoxController : MonoBehaviour
{
    ...
    //should get this new position every frame player is pushing object
    private Vector3 GetNewPushPointPosition()
    {
        Vector3 _tmpPos;
        _tmpPos = new Vector3(pushPoint.transform.position.x, pushPoint.transform.position.y, pushPoint.transform.position.z);
       return _tmpPos;
    }

    public void ParentToPushPoint()
    {
        //transform.parent = pushPoint.transform; 
        transform.SetParent(pushPoint.transform, true);
        //target.transform.parent = pushPoint.transform;
    ...
    private void Movecube()
    {
        ...
        #region old stuff that worked but am replacing
        //target.transform.position += _direction * Time.deltaTime; //og movement
        //target.GetComponent<Rigidbody>().MovePosition(target.GetComponent<Rigidbody>().position + _direction * Time.deltaTime * 5f);
        #endregion

        pushPoint.GetComponent<Rigidbody>().MovePosition(pushPoint.transform.position + _direction * Time.deltaTime * 6f); //2-5 for final floar (strength facotr)
    ...
    private void GetDirection(Vector3 _camForward, Vector3 _camRight, float _z, float _h)
    {
        ...
        if (pushPoint.transform.position.x % 2 == 0)
        {
            ...
            //target.transform.position.z % 2 != 0
            else
            {
                print("2");
        ...
        else if (pushPoint.transform.position.z % 2 == 0)
        {
            ...
            //target.transform.position.x % 2 != 0
            else
            {
                print("3");
        ...
        else
        {
            ...
            //target.GetComponent<BlockBehavior>().RoundCubeLocation();
            //target.GetComponent<BlockBehavior>().velocity = target.GetComponent<BlockBehavior>().velocity * 0;
